...
export function convertTldrawShapeToFocusedType(
  shape: TLShape,
): FocusedShape["_type"] {
  switch (shape.type) {
    case "geo": {
...
function convertDrawShapeToFocused(
  editor: Editor,
  shape: TLDrawShape,
): FocusedDrawShape {
  return {
...
function convertImageShapeToFocused(
  editor: Editor,
  shape: TLImageShape,
): FocusedImageShape {
...
function convertTextShapeToFocused(
  editor: Editor,
  shape: TLTextShape,
): FocusedTextShape {
...
function convertGeoShapeToFocused(
  editor: Editor,
  shape: TLGeoShape,
): FocusedGeoShape {
  const util = editor.getShapeUtil(shape);
...
function convertLineShapeToFocused(
  editor: Editor,
  shape: TLLineShape,
): FocusedLineShape {
  const bounds = getSimpleBounds(editor, shape);
...
function convertArrowShapeToFocused(
  editor: Editor,
  shape: TLArrowShape,
): FocusedArrowShape {
  const bounds = getSimpleBounds(editor, shape);
...
function convertNoteShapeToFocused(
  editor: Editor,
  shape: TLNoteShape,
): FocusedNoteShape {
  const util = editor.getShapeUtil(shape);
...
function convertUnknownShapeToFocused(
  editor: Editor,
  shape: TLShape,
): FocusedUnknownShape {
  const bounds = getSimpleBounds(editor, shape);
